...
public:
    ...
    increment(std::size_t workRequestedInc, std::size_t workDoneInc) {
        ...
        do {
            ...
            std::uint32_t workRequested = (oldCounter >> 32);
            std::uint32_t workDone      = oldCounter & 0xFFFFFFFF;
...
struct node : protected std::tuple<Arguments...> {
    ...
    alignas(hardware_destructive_interference_size) std::shared_ptr<fair::thread_pool::BasicThreadPool> ioThreadPool = std::make_shared<fair::thread_pool::BasicThreadPool>(
            "node_thread_pool", fair::thread_pool::TaskType::IO_BOUND);
...
public:
    ...
    work(std::size_t requested_work = std::numeric_limits<std::size_t>::max()) noexcept {
        ...
        if constexpr (is_blocking) {
            ...
            if (ioThreadRunning.compare_exchange_strong(expectedThreadState, true, std::memory_order_acq_rel)) {
...
struct fmt::formatter<fair::graph::work_return_status_t> {
    ...
    format(const fair::graph::work_return_status_t &status, FormatContext &ctx) {
        switch (status) {
        case fair::graph::work_return_status_t::ERROR: return fmt::format_to(ctx.out(), "ERROR");
        case fair::graph::work_return_status_t::INSUFFICIENT_OUTPUT_ITEMS: return fmt::format_to(ctx.out(), "INSUFFICIENT_OUTPUT_ITEMS");
        case fair::graph::work_return_status_t::INSUFFICIENT_INPUT_ITEMS: return fmt::format_to(ctx.out(), "INSUFFICIENT_INPUT_ITEMS");
        case fair::graph::work_return_status_t::DONE: return fmt::format_to(ctx.out(), "DONE");
        case fair::graph::work_return_status_t::OK: return fmt::format_to(ctx.out(), "OK");
